...
public class ReviewController {
    ...
    public String addReviewReply(ReviewReplyDTO replyDTO, HttpSession session,
        ...
        try {
            if (!rService.addReviewReply(replyDTO)) {
                redirectAttributes.addFlashAttribute("errorMessage", "답글을 등록할 수 없는 리뷰입니다.");
            } else {
                redirectAttributes.addFlashAttribute("message", "답글을 등록했습니다.");
            }
    ...
    public String modifyReviewReply(ReviewReplyDTO replyDTO, HttpSession session,
        ...
        replyDTO.setAuthorCode(AuthorSessionUtils.getAuthorCode(session));
        try {
            if (!rService.modifyReviewReply(replyDTO)) {
                redirectAttributes.addFlashAttribute("errorMessage", "수정할 수 없는 답글입니다.");
            } else {
                redirectAttributes.addFlashAttribute("message", "답글을 수정했습니다.");
            }
        } catch (IllegalArgumentException exception) {
            redirectAttributes.addFlashAttribute("errorMessage", exception.getMessage());
        }
    ...
    public String removeReviewReply(
        ...
        if (!rService.removeReviewReply(AuthorSessionUtils.getAuthorCode(session), reviewCode)) {
            redirectAttributes.addFlashAttribute("errorMessage", "삭제할 수 없는 답글입니다.");
        } else {
            redirectAttributes.addFlashAttribute("message", "답글을 삭제했습니다.");
        }
